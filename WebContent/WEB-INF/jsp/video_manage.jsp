	    ...
	    <div id="page-wrapper">
	        <div class="graphs">
		        ...
		        <div class="row" style="height: 444px;">
		           <table class="table" style="margin-top: -24px;" id="videoTable">
				      <thead>
				        <tr>
				          <th>ID</th>
				          <th>标题</th>
				      ...
				      <tbody>
				        <c:forEach items="${video_list}" var="n">
		                    <tr class="active">
			                    <th scope="row">${n.id }</th>
				          	    <td><a href="<%=PathUtil.getFullPath("video/get-video")%>?id=${n.id }">${n.title}</a></td>
				                <td>${n.updateTime}</td>
				                <td><a href="<%=PathUtil.getFullPath("video/video-load")%>?name=${n.video }"><i class="glyphicon glyphicon-arrow-down" style="margin-left: 20px;"></i></a><a href="#"><i class="glyphicon glyphicon-remove" style="margin-left: 20px;"></i></a></td>
			                </tr>
			            </c:forEach>
				      </tbody>
				    </table>
    ...
    <script type="text/javascript">
	    jQuery(document).ready(function($) {
	    	...
	    	$('#multiselect').change(function(){
				...
				$.ajax({
		             type: "GET",
		             url: "/EarlyEducation/page/video/video-list/" + rangeId + "/" + typeId,
		             dataType:"json",
		             async:false,  
		             cache:false,
		             success: function(data){
